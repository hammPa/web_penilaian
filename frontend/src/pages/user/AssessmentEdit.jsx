...
export default function AssessmentEdit() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [assessmentData, tableData, critData, varData] = await Promise.all([
        ...
        const filteredTables = tableData.filter(t => t.sessionId === assessmentData.sessionId);
        setTables(filteredTables);
        setCriteria(critData);
        setVariables(varData);

        // Prefill selections dari data assessment yang sudah ada
        const initial = {};
        varData.forEach(v => { initial[v.id] = null; });
        (assessmentData.selections || []).forEach(s => {
          initial[s.variableId] = s.selectedLevel;
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(err.response?.data?.message || err?.message || 'Gagal menyimpan perubahan', 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (!assessment || tables.length === 0 || criteria.length === 0 || variables.length === 0) {
    return <EmptyState message="Data penilaian tidak tersedia atau tidak lengkap" />;
  }
  ...
  groupedTables.forEach(t => {
    t.criteria.forEach(c => { totalCount += c.variables.length; });
  });
  ...
  return (
    <div className="min-h-full bg-[#F3F4F7] -m-6 p-6 md:-m-8 md:p-8 pb-32 md:pb-36">
      <div className="mb-6">
        <p className="text-[10px] sm:text-[11px] font-medium uppercase tracking-[0.14em] text-[#C8933E]">Edit</p>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Widget Upload Foto Dokumentasi (foto lama + foto baru) */}
        <div className="mt-6 sm:mt-8 bg-white p-5 sm:p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="grid grid-cols-3 sm:flex sm:flex-wrap gap-3">
            ...
            {previews.map((src, index) => (
              <div key={`new-${index}`} className="relative aspect-square sm:w-24 sm:h-24 rounded-lg overflow-hidden border border-slate-200 shadow-sm">
          ...
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-1 min-w-0 max-w-md">
              ...
              <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden">
                <div className="h-full rounded-full bg-[#C8933E] transition-all duration-300" style={{ width: `${progressPct}%` }} />
              </div>
            </div>
