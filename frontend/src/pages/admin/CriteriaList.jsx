...
export default function CriteriaList() {
  ...
  const [form, setForm] = useState({ name: '', weight: 1, description: '' });
  ...
  const fetchCriteria = async () => {
    try {
      ...
      showToast('Gagal memuat kriteria', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (item) => {
    setEditItem(item);
    setForm({ name: item.name, weight: item.weight, description: item.description });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.name || form.weight === '') {
      showToast('Nama dan bobot wajib diisi', 'error');
      return;
    }
    try {
      if (editItem) {
  ...
  return (
    <div>
      ...
      {criteria.length === 0 ? (
        <EmptyState message="Belum ada kriteria" icon=<ClipboardList /> />
      ) : (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <Table
            headers={['Nama', 'Bobot', 'Deskripsi', 'Aksi']}
            data={criteria}
            renderRow={(item) => (
